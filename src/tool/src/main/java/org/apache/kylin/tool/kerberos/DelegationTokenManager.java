...
public class DelegationTokenManager {
    ...
    private void addCredentials() throws IOException, NoSuchFieldException {
        ...
        Collection<Object> cacheKeys = getCacheKeys(fsCache);
        ...
        for (Object key : cacheKeys) {
            ...
            if (updatable(ugi, current)) {
                updateUGICreds(ugi, creds);
            }
        }
    }

    private void updateUGICreds(UserGroupInformation ugi, Credentials creds) {
        try {
            final Collection<Token<? extends TokenIdentifier>> tokens = creds.getAllTokens() //
                    .stream().filter(t -> Objects.nonNull(t.getKind()) && Objects.nonNull(t.getService())) //
    ...
    private boolean updatable(UserGroupInformation ugi, UserGroupInformation current) {
        if (Objects.isNull(ugi) || Objects.isNull(current) || ugi.equals(current)) {
            return false;
        }

        // Project level kerberos separation.
        if (Objects.nonNull(ugi.getUserName()) && !ugi.getUserName().equals(current.getUserName())) {
            logger.debug("UGI username is different with current username: {} vs {}", //
                    ugi.getUserName(), current.getUserName());
            return false;
        }

        final List<AbstractDelegationTokenIdentifier> tokens = current.getTokens().stream() //
                .map(this::getDelegationTokenIdentifier) //
        ...
        final List<AbstractDelegationTokenIdentifier> olds = ugi.getTokens().stream() //
                .map(this::getDelegationTokenIdentifier) //
        ...
        return tokens.stream().anyMatch(dt -> //
                olds.stream().anyMatch(odt -> Objects.nonNull(odt.getKind()) //
                        && odt.getKind().equals(dt.getKind()) //
                        && odt.getSequenceNumber() < dt.getSequenceNumber()));
    }

    private AbstractDelegationTokenIdentifier getDelegationTokenIdentifier(Token<? extends TokenIdentifier> token) {
        try {
            TokenIdentifier ti = token.decodeIdentifier();
            if (ti instanceof AbstractDelegationTokenIdentifier) {
                return (AbstractDelegationTokenIdentifier) ti;
            }
        } catch (IOException e) {
    ...
    private void updatePrivateTokens(UserGroupInformation ugi, //
                                     Collection<Token<? extends TokenIdentifier>> tokens) throws NoSuchMethodException, NoSuchFieldException {
        final Credentials creds = getCredentialsInternal(ugi);
        if (Objects.isNull(creds)) {
            return;
        }
        final Collection<Text> keys = getTokenKeys(creds);
        if (keys.isEmpty()) {
            return;
        }
        for (Token<? extends TokenIdentifier> token : tokens) {
            final Text kind = token.getKind();
            final Text service = token.getService();
            final AbstractDelegationTokenIdentifier dt = getDelegationTokenIdentifier(token);
            keys.forEach(key -> {
                Token<? extends TokenIdentifier> otk = creds.getToken(key);
                if (Objects.isNull(otk) //
                        || Objects.isNull(otk.getKind()) //
                        || Objects.isNull(otk.getService()) //
                        || !otk.getKind().equals(kind) //
                        || otk.getService().equals(service)) {
                    return;
                }

                final AbstractDelegationTokenIdentifier odt = getDelegationTokenIdentifier(otk);
                if (Objects.nonNull(dt) && Objects.nonNull(odt) //
                        && dt.getSequenceNumber() <= odt.getSequenceNumber()) {
                    return;
                }

                final Token<? extends TokenIdentifier> tk = new Token<>(token);
    ...
    private Collection<Text> getTokenKeys(Credentials creds) throws NoSuchFieldException {
        Field mapFiled = Credentials.class.getDeclaredField("tokenMap");
        ReflectionUtils.makeAccessible(mapFiled);
        Map<Text, Object> tokenMap = (Map<Text, Object>) ReflectionUtils.getField(mapFiled, creds);
        if (Objects.isNull(tokenMap)) {
            return Collections.emptyList();
        }
        // Readable only.
        return Collections.unmodifiableCollection(tokenMap.keySet());
    }
