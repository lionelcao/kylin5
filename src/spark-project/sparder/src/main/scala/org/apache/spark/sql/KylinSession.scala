  ...
  implicit class KylinBuilder(builder: Builder) {
    ...
    def initSparkConf(sparkConf: SparkConf): SparkConf = {
      ...
      if (kapConfig.getKylinConfig.asyncProfilingEnabled()) {
        ...
        if (plugins.isEmpty) {
          sparkConf.set("spark.plugins", "io.kyligence.kap.query.asyncprofiler.QueryAsyncProfilerSparkPlugin")
        } else {
          sparkConf.set("spark.plugins", "io.kyligence.kap.query.asyncprofiler.QueryAsyncProfilerSparkPlugin," + plugins)
        }
      }
